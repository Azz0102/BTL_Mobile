    ...
    <SafeAreaView className="flex-1 pb-20">
      <ScrollView className="px-2 pt-1 flex-1" stickyHeaderIndices={[2]}>
        <View className="p-2 flex-row justify-between items-center">
          <View>
            <Text className="text-lg text-gray-500">Hi Foodie,</Text>
          ...
          <ScrollView
            ...
            {categories.map(category => (
              <TouchableOpacity
                ...
                <Text className={filter === category.tag ? 'text-white' : ''}>
                  {category.title}
                </Text>
              </TouchableOpacity>
            ))}
        ...
        {filterfoodData?.length === 0 && (
          <Text className="text-gray-400 text-center">No Items Found...</Text>
        )}
        {isLoading && (
